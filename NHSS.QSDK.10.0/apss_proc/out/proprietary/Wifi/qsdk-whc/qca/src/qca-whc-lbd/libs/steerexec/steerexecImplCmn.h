...
typedef struct steerexecImplCmnLegacyConfig_t {
    ...
    /// steering mechanism.
    u_int32_t steeringProhibitTime[BSTEERING_MAX_CLIENT_CLASS_GROUP];
    ...
    u_int32_t authRejMax[BSTEERING_MAX_CLIENT_CLASS_GROUP];

    /// The base amount of time a device should be considered as steering
    /// unfriendly before being attempted again (s).
    u_int32_t steeringUnfriendlyTime[BSTEERING_MAX_CLIENT_CLASS_GROUP];
    ...
    u_int32_t maxSteeringUnfriendly[BSTEERING_MAX_CLIENT_CLASS_GROUP];
    ...
    LBD_BOOL disableLegacySteering[BSTEERING_MAX_CLIENT_CLASS_GROUP];

    /// The amount of time a device should be considered as legacy upgrade steering
    /// unfriendly before being attempted again.
    u_int32_t legacyUpgradeUnfriendlyTime[BSTEERING_MAX_CLIENT_CLASS_GROUP];
...
typedef struct steerexecImplCmnBTMConfig_t {
    ...
    u_int32_t steeringProhibitShortTime[BSTEERING_MAX_CLIENT_CLASS_GROUP];
...
typedef struct steerexecImplCmnConfig_t {
    ...
    /// for association
    u_int8_t targetLowRSSIThreshold[wlanif_band_invalid][BSTEERING_MAX_CLIENT_CLASS_GROUP];
...
typedef enum steerexecImplCmnSteeringStatusType_e {
    ...
    /// Steer was aborted due to BSS mismatch on the target channel
 ...
 *                                      not eligible for
 *                                      steering
 * @param [in] smartMonitorEligible LBD_TRUE if steering eligibility
                                    is determined through smart 
                                    monitoring.
...
/**
 * @brief Check whether the steering is subject to steering attempt limit
 *
 * @param [in] reason  The reason for the steer
 *
 * Return LBD_TRUE if the steering is subject to steering attempt limit
 */
LBD_BOOL steerexecImplIsSubjectToSteeringLimit(steerexec_reason_e reason);

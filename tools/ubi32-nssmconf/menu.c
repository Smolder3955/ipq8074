...
static void prop_error(struct property *prop, const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	fprintf(stderr, "%s:%d:warning: ", prop->file->name, prop->lineno);
...
void sym_check_prop(struct symbol *sym)
{
	...
	for (prop = sym->prop; prop; prop = prop->next) {
		switch (prop->type) {
		case P_DEFAULT:
			if ((sym->type == S_STRING || sym->type == S_UQSTRING || sym->type == S_INT || sym->type == S_HEX) &&
		...
		if (sym && sym_is_choice(sym) && menu->sym) {
			...
			for (prop = menu->sym->prop; prop; prop = prop->next) {
				if (prop->type == P_PROMPT && prop->menu != menu) {
					/* 
					 * This warning doesn't break proper work. Comment it
					 * not to disturb menuconfig users. 
					 */

...
int pld_register_driver(struct pld_driver_ops *ops)
{
	...
	if(!qca6290_support) {
		...
		if (ret) {
			printk("Fail to register ahb driver\n");
			goto fail_ahb;
		}
		pld_context->pld_driver_state |= 1 << PLD_AHB_REGISTERED;
	}

	if(qca6290_support) {
		ret = pld_pcie_register_driver();
		if (ret) {
			printk("Fail to register pcie driver\n");
			goto fail_pcie;
		}
		pld_context->pld_driver_state |= 1 << PLD_PCIE_REGISTERED;
	}
#if 0
		    ...
		    enum pld_driver_mode mode, const char *host_version)
{
	...
	case PLD_BUS_TYPE_AHB:
		ret = pld_ahb_wlan_enable(dev, config, mode, host_version);
		break;
...
int pld_is_fw_ready(struct device *dev)
{
	return cnss_is_fw_ready(dev);
}
int pld_is_cold_boot_cal_done(struct device *dev)
{
	return cnss_is_cold_boot_cal_done(dev);
}

...
#define ieee1905GetRxInterface(_frame)                        \
    (((ieee1905Message_t *)_frame)->ieee1905Header.reserved + \
     ((((ieee1905Message_t *)_frame)->ieee1905Header.flags & 0x3F) * 256))
...
/**
 * @brief Determine if adding a new TLV of the specified length would
 *        exceed the maximum Ethernet frame size and thus require
 *        fragmentation.
 *
 * @param [in] curLen  the current number of bytes consumed in the frame
 *                     (including the header)
 * @param [in] tlvPayloadLen  the number of bytes in the payload of the
 *                            TLV to be added
 *
 * @return IEEE1905_TRUE if it will fit (including a subsequent end TLV);
 *         othewise IEEE1905_FALSE
 */

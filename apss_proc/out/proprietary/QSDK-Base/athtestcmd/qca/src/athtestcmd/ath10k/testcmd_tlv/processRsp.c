...
void parseResNart()
{
    A_BOOL ret = TRUE;
    A_UINT8 *data;
    A_UINT32 status, commandId, len, value;
    A_UINT8  buf[2048];
    A_UINT32 i;
...
void handleTPCCALRsp(CMD_TPCCALRSP_PARMS *pParms)
{
    int i;
    printf("radioID %d, numFreq2G %d, numFreq5G %d, numChain %d, numCalPt %d\n", pParms->radioId,
        pParms->numFreq2G, pParms->numFreq5G, pParms->numChain, pParms->numCALPt2G);
    printf("freq2G: ");
    for (i=0;i<pParms->numFreq2G;i++) {
        printf("%d ", pParms->freq2G[i]);
    }
    printf("\n");
    printf("freq5G: ");
    for (i=0;i<pParms->numFreq5G;i++) {
        printf("%d ", pParms->freq5G[i]);
    }
    printf("\n");
}

void handleTPCCALDATA(CMD_TPCCALDATA_PARMS *pParms)
{
#ifdef EEPROM_HEADER
    FILE *fp;
    size_t numBytes;
...
void handleRXRSP (void *parms)
{
    ...
    printf("totalpkt %d\n", pParms->totalPackets);
    printf("rssiInDBm %d\n", (pParms->totalPackets ? (pParms->rssiInDBm1/(A_INT32)pParms->totalPackets) : pParms->rssiInDBm1));
    printf("crcErrPkt %d\n", pParms->crcPackets);
    printf("secErrPkt %d\n", pParms->secErrPkt);
    if (pParms->shortGuard ) printf("<<<<SGI set >>>>\n");
...
void parseResRxStatus()
{
    A_INT32 totalpkt, goodPackets, otherError, crcPackets, numOfReports=0;
    A_INT32 decrypErrors, rateBit, row, maskColumn;
    A_INT32 rssi, ret, len;
    A_INT32 rssic0, rssic1, rssic2, rssic3;
    A_INT32 rssie0, rssie1, rssie2, rssie3;
    A_INT32 noisefloor0 = 0, noisefloor1 = 0, noisefloor2 =0, noisefloor3 =0, nf_comb=0;
    A_UINT8 *data = NULL;

    ret = getParams((A_UINT8*)"numOfReports", &data, &len);

    if ( ret == TRUE )
    {
        memcpy(&numOfReports,data,len);
    }else {
        printf("\nFailed ot get reports .... \n");
    }

    if(numOfReports) {
        ret = getParams((A_UINT8*)"totalPackets", &data, &len);
        ...
        ret = getParams((A_UINT8*)"goodPackets", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&goodPackets,data,len);
            printf("goodPackets %d\n",goodPackets);
        }

        ret = getParams((A_UINT8*)"otherError", &data, &len);
        ...
        ret = getParams((A_UINT8*)"crcPackets", &data, &len);
        ...
        ret = getParams((A_UINT8*)"decrypErrors", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&decrypErrors,data,len);
            printf("decrypErrors %d\n",decrypErrors);
        }

        ret = getParams((A_UINT8*)"rateBit", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rateBit,data,len);
            row =  rateBit/32;
            if(row > 0) 
                maskColumn = rateBit - (32 * row);
            else 
                maskColumn = rateBit;
            printf("rateBit %d datarate %d \n",rateBit, Mask2Rate[row][maskColumn]);
        }

        ret = getParams((A_UINT8*)"noisefloor0", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&noisefloor0,data,len);
        }else {
        //    printf("Parse failed\n");
        }

        ret = getParams((A_UINT8*)"noisefloor1", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&noisefloor1,data,len);
        }

        ret = getParams((A_UINT8*)"noisefloor2", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&noisefloor2,data,len);
        }

        ret = getParams((A_UINT8*)"noisefloor3", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&noisefloor3,data,len);
        }

        ret = getParams((A_UINT8*)"rssi", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssi,data,len);
            nf_comb = (noisefloor0 + noisefloor1 + noisefloor2 + noisefloor3)/4;
            if(rssi) 
                rssi = rssi - nf_comb;
            printf("RSSI combined in dBm %d\n",rssi);
        }

        ret = getParams((A_UINT8*)"rssic0", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssic0,data,len);
            if(rssic0)
                rssic0 = rssic0 - noisefloor0;
            printf("RSSI in dBm chain 0 %d\n",rssic0);
        }

        ret = getParams((A_UINT8*)"rssic1", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssic1,data,len);
            if(rssic1)
                rssic1 = rssic1 - noisefloor1;
            printf("RSSI in dBm chain 1 %d\n",rssic1);
        }

        ret = getParams((A_UINT8*)"rssic2", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssic2,data,len);
            if(rssic2)
                rssic2 = rssic2 - noisefloor2;
            printf("RSSI in dBm chain 2 %d\n",rssic2);
        }

        ret = getParams((A_UINT8*)"rssic3", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssic3,data,len);
            if(rssic3)
                rssic3 = rssic3 - noisefloor3;
            printf("RSSI in dBm chain 3 %d\n",rssic3);
        }

        ret = getParams((A_UINT8*)"rssie0", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssie0,data,len);
            printf("rssi ext sec 20 chain 0 %d\n",rssie0);
        }

        ret = getParams((A_UINT8*)"rssie1", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssie1,data,len);
            printf("rssi ext sec 20 chain 1 %d\n",rssie1);
        }

        ret = getParams((A_UINT8*)"rssie2", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssie2,data,len);
            printf("rssi ext sec 20 chain 2 %d\n",rssie2);
        }

        ret = getParams((A_UINT8*)"rssie3", &data, &len);

        if ( ret == TRUE )
        {
            memcpy(&rssie3,data,len);
            printf("rssi ext sec 20 chain 3 %d\n",rssie3);
        }
    }else {
        printf("\nNo reports received...\n");
    }

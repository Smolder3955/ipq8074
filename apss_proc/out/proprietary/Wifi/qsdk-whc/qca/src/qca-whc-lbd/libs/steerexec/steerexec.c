...
steerexec_steerEligibility_e steerexec_determine11kUnfriendlySteeringEligibility(
    stadbEntry_handle_t entry) {
    // Underlying steering orchestrator must check parameters itself.
    if (!steerexecState.steerexecImplCmn) {
        return steerexec_steerEligibility_none;
...
LBD_BOOL steerexec_isSubjectToSteeringLimit(steerexec_reason_e reason) {
    return steerexecImplIsSubjectToSteeringLimit(reason);
}
...
static LBD_STATUS steerexecReadConfig(steerexecImplCmnConfig_t *config,
    ...
    if (profileGetOptsIntArray(mdModuleID_SteerExec,
        STEEREXEC_STEERING_PROHIBIT_TIME_KEY,
        steerexecElementDefaultTable, value) == LBD_NOK) {
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->legacy.steeringProhibitTime[index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->legacy.authRejMax[index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->legacy.steeringUnfriendlyTime[index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->legacy.maxSteeringUnfriendly[index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->legacy.disableLegacySteering[index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->targetLowRSSIThreshold[wlanif_band_24g][index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->targetLowRSSIThreshold[wlanif_band_5g][index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->btm.steeringProhibitShortTime[index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        config->legacy.legacyUpgradeUnfriendlyTime[index] = value[index];
    }

    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        if (config->btm.steeringProhibitShortTime[index] >=
            config->legacy.steeringProhibitTime[index]) {
            dbgf(dbgModule, DBGERR,
                 ...
                 __func__, config->btm.steeringProhibitShortTime[index],
                 config->legacy.steeringProhibitTime[index]);
            return LBD_NOK;
        }
    }

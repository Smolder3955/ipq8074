...
typedef struct steeralgPriv_t {
    ...
    struct {
        ...
        lbd_rssi_t inactRSSIXingThreshold[wlanif_band_invalid][BSTEERING_MAX_CLIENT_CLASS_GROUP];

        /// Low Tx rate threshold on 5 GHz (Mbps)
        lbd_linkCapacity_t lowTxRateThreshold;

        /// High Tx rate threshold on 2.4 GHz (Mbps)
        lbd_linkCapacity_t highTxRateThreshold[BSTEERING_MAX_CLIENT_CLASS_GROUP];
        ...
        u_int8_t rssiSafetyThreshold[BSTEERING_MAX_CLIENT_CLASS_GROUP];

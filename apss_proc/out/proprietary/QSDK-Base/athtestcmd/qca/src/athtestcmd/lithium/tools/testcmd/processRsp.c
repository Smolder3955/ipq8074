...
void parseTLVSubmitReport()
{
...
#ifdef WIN32
        sprintf_s((char*)rateCntBuf, sizeof(rateCntBuf), "%s%d","rcMask",i);
#else
    ...
    for(i=0;i<4;i++)
    {
        memset(rateCntBuf,0,sizeof(rateCntBuf));
#ifdef WIN32
            sprintf_s((char*)rateCntBuf, sizeof(rateCntBuf), "%s%d","rcMask11ac",i);
#else
    ...
    for (k=0;k<2;k++)
    {
        ...
        while (rateMask)
        {
            if (bit0 & rateMask)
            {
                ...
                memset(rateCntBuf,0,sizeof(rateCntBuf));
#ifdef WIN32
                sprintf_s((char*)rateCntBuf, sizeof(rateCntBuf), "%s%d","rateCnt",index);
#else
    ...
    for (k=0;k<4;k++)
    {
        ...
        while (rateMask)
        {
            if (bit0 & rateMask)
            {
                ...
                memset(rateCntBuf,0,sizeof(rateCntBuf));
#ifdef WIN32
                sprintf_s((char*)rateCntBuf, sizeof(rateCntBuf), "%s%d","rateCnt11ac", index);
#else
    ...
    for (k=0;k<1;k++)
    {
        ...
        while (rateMask)
        {
            if (bit0 & rateMask)
            {
                ...
                memset(rateCntBuf,0,sizeof(rateCntBuf));
#ifdef WIN32
                sprintf_s((char*)rateCntBuf, sizeof(rateCntBuf), "%s%d","rtCntac4x4",index);
#else
...
void handleTPCCALRsp(void *pParms1)
{
	CMD_TPCCALRSP_PARMS *pParms = (CMD_TPCCALRSP_PARMS *)pParms1;
    int i;
...
void handleTPCCALDATA(void *pParms)
{
#ifdef EEPROM_HEADER
	CMD_TPCCALDATA_PARMS *pParms = (CMD_TPCCALDATA_PARMS *)pParms1;
    FILE *fp;
    size_t numBytes;
    ...
    pEepStruct->baseBdfHeader.checksum = 0x0000;
    pHalf = (A_UINT16 *)pEepStruct;
    sum = 0;
    length=sizeof(eepromData) ; 
    for (i = 0; i < (length/2); i++) { sum ^= *pHalf++; }
    pEepStruct->baseBdfHeader.checksum = 0xFFFF ^ sum;
...
void handleMEMREADRSP(void *parms)
{
    ...
    if (pParms->status == 0)
    {
        ...
        {
            if (pParms->valuetype == 1)
            {
                printf("mem[%x]=0x%02x", pParms->memaddress + i, pParms->memvalue[i]);
            }
            else if (pParms->valuetype == 2)
            {
                printf("mem[%x]=0x%04x", pParms->memaddress + i, *((A_UINT16 *)&pParms->memvalue[i]));
                ++i;
            }
            else
            {
                printf("mem[%x]=0x%08x", pParms->memaddress + i, *((A_UINT32 *)&pParms->memvalue[i]));
                i += 3;
            }
...
void handleTestDataRsp (void *parms)
{
	SYSCMD_TESTDATARSP_PARMS *pParms = (SYSCMD_TESTDATARSP_PARMS *)parms;
    A_UINT32 i, status = 0;

    if (parms == NULL || (pParms->status32 == CMD_TLV_MORE_SEGMENT))
    {
        return; //do nothing
    }
    ...
    for (i = 0; i < 1024; i++)
    {
        if (pParms->data1[i] != 0xaa)
        {
            status = (1 << 8) | ((i << 16) & 0xffff0000) | (pParms->data1[i] & 0xff);
            printf("0x%08x\n", status);
            return;
        }
    }
    for (i = 0; i < 1024; i++)
    {
        if (pParms->data2[i] != 0x55)
        {
            status = (2 << 8) | ((i << 16) & 0xffff0000) | (pParms->data2[i] & 0xff);
            printf("0x%08x\n", status);
            return;
        }
    }
    for (i = 0; i < 1024; i++)
    {
        if (pParms->data3[i] != 0xa5)
        {
            status = (3 << 8) | ((i << 16) & 0xffff0000) | (pParms->data3[i] & 0xff);
            printf("0x%08x\n", status);
            return;
        }
    }
    for (i = 0; i < 1024; i++)
    {
        if (pParms->data4[i] != 0x5a)
        {
            status = (4 << 8) | ((i << 16) & 0xffff0000) | (pParms->data4[i] & 0xff);
            printf("0x%08x\n", status);
            return;
        }
    }
...
void handleMoreSegment (void *parms)
{
	...
	if (pCmdStream)
	{
        ...
        netmodule_send(handle, rCmdStream, cmdStreamLen);
#else
        //todo
#endif //USE_NET_MODULE
    }
...
void handleMoreSegmentRsp (void *parms)
{
    //handled in athtestcmd.c
}
...
void handleBdReadRsp (void *parms)
{
    ...
    char fileName[] = "tempBd.bin";
    A_UINT32 writeSize;
    static FILE *fp;

    if (parms == NULL || (pParms->status == CMD_TLV_MORE_SEGMENT))
    {
        printf("handleBdReadRsp: more segment\n");
        return; //do nothing
    }
    if (pParms->status != CMD_OK)
    {
    ...
    if ((writeSize = fwrite(pParms->data4k, 1, pParms->size, fp)) != pParms->size)
    {
        printf("Error in writing to %s - expected %d but written %d \n", fileName, pParms->size, writeSize);
        return;
    }
    fclose(fp);
}

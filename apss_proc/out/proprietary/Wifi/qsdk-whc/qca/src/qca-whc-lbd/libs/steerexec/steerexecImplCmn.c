...
typedef struct steerexecImplCmnCheckChannelRSSI_t {
    ...
    /// is BSS on a valid channel
    LBD_BOOL isBSSOnValidChannel;
...
steerexec_steerEligibility_e steerexecImplCmnDetermineSteeringEligibility(
    ...
    if (!state) {
        ...
        if (exec->config.btm.startInBTMActiveState &&
            ...
            (smartMonitorEligible || stadbEntry_isRRMSupported(entry))) {
            // As long as the STA supports BTM and RRM/Smart Monitor,
            // it can be steered while active.
    ...
    if (steerexecImplCmnIsLegacySteer(steerType)) {
        ...
        if (state->btm->complianceState == steerexecImplCmn_btmComplianceState_active) {
            // Determine if the STA supports RRM or AP supports Smart Monitor
            if (smartMonitorEligible || stadbEntry_isRRMSupported(entry)) {
                return steerexec_steerEligibility_active;
            } else {
                // Even though the STA could be steered while active according
                // to steerexec, it doesn't support RRM or smart monitor is not
                // exercised, so we can't get stats on the non-serving channel.
                // Only attempt to steer while idle.
                return steerexec_steerEligibility_idle;
            }
        } else {
            return steerexec_steerEligibility_idle;
...
static steerexecImplCmnSteeringType_e steerexecImplCmnDetermineSteeringType(
    ...
    if (!stadbEntry_isBTMSupported(entry)) {
        ...
        steerType = steerexecImplCmnDetermineLegacySteeringType(isBestEffort,
                        exec->config.legacy.disableLegacySteering[clientClassGroup]);
    } else {
        if (eligibilityOnly) {
...
static u_int32_t steerexecImplCmnGetSteeringProhibitTime(
    ...
    if (prohibit == steerexecImplCmnSteeringProhibitType_short) {
        return exec->config.btm.steeringProhibitShortTime[clientClassGroup];
    } else if (prohibit == steerexecImplCmnSteeringProhibitType_long) {
        return exec->config.legacy.steeringProhibitTime[clientClassGroup];
...
static void steerexecImplCmnStartSteeringProhibitLocal(
    ...
    u_int32_t prohibitTime =
        steerexecImplCmnGetSteeringProhibitTime(exec, prohibit, clientClassGroup) + 1;
...
static void steerexecImplCmnStartSteeringProhibit(
    ...
    u_int32_t configured_prohibitTime;
    u_int8_t clientClassGroup = 0;
    ...
    configured_prohibitTime =
        steerexecImplCmnGetSteeringProhibitTime(exec, steerexecImplCmnSteeringProhibitType_long, clientClassGroup);

    // Check for corrupted prohibit time:
    // Calculated prohitbit time can be larger than actual configured value but cannot be
    // larger than twice the longer prohibit value configured
    if( prohibitTime > configured_prohibitTime*2)
    {
        struct timespec ts;
        lbGetTimestamp(&ts);

        dbgf(state->context->dbgModule, DBGERR,
            "%s: Prohibit Timer is Corrupted.(value = %u) Resetting to Short Prohibit Time", __func__, prohibitTime);

        prohibitTime = steerexecImplCmnGetSteeringProhibitTime(exec, steerexecImplCmnSteeringProhibitType_short, clientClassGroup);
        state->legacy.prohibitExpiryTime = ts.tv_sec + prohibitTime;
...
static void steerexecImplCmnStartSteeringUnfriendly(
    ...
    steerexecImplCmnStartExpBackoff(exec->dbgModule,
                                    ...
                                    exec->config.legacy.steeringUnfriendlyTime[clientClassGroup],
                                    &state->legacy.unfriendlyExpiryTime,
                                    state->legacy.countConsecutiveFailure,
                                    exec->config.legacy.maxSteeringUnfriendly[clientClassGroup]);
        ...
        time_t *expiryTime)
{
    ...
    if (!exec->config.legacy.legacyUpgradeUnfriendlyTime[clientClassGroup]) {
        return;
    }
    state->legacy.legacyUpgradeUnfriendly = LBD_TRUE;
    timer->countEntries++;
    baseTime = exec->config.legacy.legacyUpgradeUnfriendlyTime[clientClassGroup] + 1;
...
static void steerexecImplCmnProhibitTimeoutHandler(void *cookie) {
    steerexecImplCmnHandle_t exec = (steerexecImplCmnHandle_t) cookie;
    u_int32_t steeringProhibitTime = 0;
    u_int8_t index = 0;

    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        if (steeringProhibitTime <
            exec->config.legacy.steeringProhibitTime[index]) {
...
static void steerexecImplCmnUnfriendlyTimeoutHandler(void *cookie) {
    steerexecImplCmnHandle_t exec = (steerexecImplCmnHandle_t) cookie;
    u_int32_t steeringUnfriendlyTime = 0;
    u_int8_t index = 0;

    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        if (steeringUnfriendlyTime <
            exec->config.legacy.steeringUnfriendlyTime[index]) {
...
static void steerexecImplCmnLegacyUpgradeUnfriendlyTimeoutHandler(void *cookie) {
    steerexecImplCmnHandle_t exec = (steerexecImplCmnHandle_t) cookie;
    u_int32_t legacyUpgradeUnfriendlyTime = 0;
    u_int8_t index = 0;

    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        if (legacyUpgradeUnfriendlyTime <
            exec->config.legacy.legacyUpgradeUnfriendlyTime[index]) {
            legacyUpgradeUnfriendlyTime =
                exec->config.legacy.legacyUpgradeUnfriendlyTime[index];
        }
    }
...
LBD_STATUS steerexecImplCmnTimeoutBlacklistCandidate(
    ...
    for (i = 0; i < blacklistCount; i++) {
        ...
        if (wlanif_setCandidateStateForSTA(1,
                                           ...
                                           lastServingESS,
                                           LBD_TRUE /* enable */,
                                           servingBSS, 0, clientClassGroup) != LBD_OK) {
            dbgf(state->context->dbgModule, DBGERR,
...
static LBD_BOOL steerexecImplCmnHandleAuthRejLocal(
    ...
    if(evloopTimeoutRemaining(&state->legacy.tSteerTimer, &secsRemaining,
        ...
        if (state->legacy.numAuthRejects == exec->config.legacy.authRejMax[clientClassGroup]) {
...
static LBD_BOOL steerexecImplCmnIsTargetChannelRSSIOK(
    ...
    if (rssi == LBD_INVALID_RSSI ||
        ...
        rssi >= exec->config.targetLowRSSIThreshold[targetBand][clientClassGroup]) {
    ...
    dbgf(exec->dbgModule, DBGDEBUG,
         ...
         __func__, rssi, channel,
         exec->config.targetLowRSSIThreshold[targetBand][clientClassGroup]);
    return LBD_FALSE;
}
...
static LBD_BOOL steerexecImplCmnIsTargetRSSIOKChannelCallback(
    ...
    if (!steerexecImplCmnIsOnChannelList(params->enabledChannelCount,
                                      ...
                                      bssInfo->channelId)) {
        params->isBSSOnValidChannel = LBD_FALSE;
        return LBD_FALSE;
    }
...
static LBD_BOOL steerexecImplCmnIsTargetRSSIOKChannel(
    ...
    if ((params.isBSSOnValidChannel == LBD_FALSE) && (params.isChannelRSSIOK == LBD_FALSE)) {
        *rssiBssMismatch = LBD_FALSE;
    }
...
static void steerexecImplCmnRSSIObserver(stadbEntry_handle_t entry,
    ...
    LBD_BOOL rssiBssMismatch = LBD_TRUE;
    if (state->steerType == steerexecImplCmnSteeringType_none) {
        return;
    } else if (state->steerType == steerexecImplCmnSteeringType_preassociation) {
        rssiSafe = steerexecImplCmnIsTargetRSSIOKChannel(exec, state, entry, &rssiBssMismatch);
    } else {
        rssiSafe = steerexecImplCmnIsTargetRSSIOKCandidate(exec, state, entry);
    }
    if (rssiBssMismatch == LBD_FALSE ) {
        steerexecImplCmnAbortSteerImpl(exec, entry, state,
                            steerexecImplCmnSteeringStatusType_channel_bss_mismatch,
                            LBD_TRUE /* isLocalAbort */);
    } else if (!rssiSafe) {
        steerexecImplCmnAbortSteerImpl(exec, entry, state,
...
static void steerexecImplCmnUpdateBlacklistOnSteerCompletion(
    ...
    if (enableCount) {
        if (wlanif_setCandidateStateForSTA(enableCount, &enableList[0],
                                           staAddr,lastServingESS, LBD_TRUE /* enable */,
                                            servingBSS, 0, clientClassGroup) != LBD_OK) {
            dbgf(state->context->dbgModule, DBGERR,
    ...
    if (!steerexecImplCmnIsBTMOnlySteer(steerType)) {
        ...
        if (updatedState) {
            ...
            if (wlanif_setNonCandidateStateForSTA(candidateCount, candidateList,
                                               ...
                                               LBD_FALSE /* probeOnly */,
                                               servingBSS, clientClassGroup) != LBD_OK) {
                dbgf(exec->dbgModule, DBGERR,
...
void steerexecImplCmnDumpLegacyHeader(struct cmdContext *context,
    ...
    lbGetTimestamp(&ts);
    u_int8_t index = 0;
    u_int8_t steeringUnfriendlyTimeInvalidEntryCount = 0;
    u_int8_t legacyUpgradeUnfriendlyTimeInvalidEntryCount = 0;
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        if (exec->config.legacy.steeringUnfriendlyTime[index] <= 0) {
            steeringUnfriendlyTimeInvalidEntryCount++;
        }
        if (exec->config.legacy.legacyUpgradeUnfriendlyTime[index] <= 0) {
            legacyUpgradeUnfriendlyTimeInvalidEntryCount++;
        }
    }
...
static void steerexecImplCmnDumpBTMEntryState(struct cmdContext *context,
    ...
    steerexec_steerEligibility_e eligibility =
        steerexecImplCmnDetermineSteeringEligibility(
            ...
            LBD_FALSE /* smartMonitorEligible */,

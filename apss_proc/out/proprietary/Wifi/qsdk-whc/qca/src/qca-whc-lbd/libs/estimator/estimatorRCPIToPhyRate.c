...
static lbd_snr_t estimatorRCPIToPhyRateEstimateSNR(struct dbgModule *dbgModule,
    ...
    if (rcpi > NOISE_OFFSET_BY_CH_WIDTH[chwidth]) {
        return rcpi - NOISE_OFFSET_BY_CH_WIDTH[chwidth];
    } else {
        // Would have underflowed, so just force the SNR to 0.
        return 0;
    }
}

int8_t estimatorRCPIToPhyRateConvertRSSIFromDbToDbm(struct dbgModule *dbgModule,
        lbd_rssi_t rssi, wlanif_chwidth_e chanWidth) {
    lbDbgAssertExit(dbgModule, chanWidth < wlanif_chwidth_invalid);
    return rssi + NOISE_OFFSET_BY_CH_WIDTH[chanWidth];
}

lbd_rssi_t estimatorRCPIToPhyRateConvertRSSIFromDbmToDb(struct dbgModule *dbgModule,
        int8_t rcpi, wlanif_chwidth_e chanWidth) {
    return estimatorRCPIToPhyRateEstimateSNR(dbgModule, rcpi, chanWidth);
}

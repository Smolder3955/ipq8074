...
static int switchCommandExcute(u_int32_t devId, ...)
{
	...
	va_end(vaArgs);
	(void)ioctl(switchWrapperState.socketFd, SIOCDEVPRIVATE, nValue);
	return nRtn;
}
...
static int switchAddIgmpMldRules()
{
	...
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, 0, 10, 0, 1, &rule);
	...
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, 0, 20, 0, 1, &rule);
	rule.icmp_type_val = 131; /*Multicast Listener Report*/
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, 0, 20, 1, 1, &rule);
	rule.icmp_type_val = 132; /*Multicast Listener Done*/
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, 0, 20, 2, 1, &rule);
	rule.icmp_type_val = 143; /*Multicast Listener Report, Version 2*/
	switchCommandExcuteVar(SW_API_ACL_RULE_ADD, 5, 0, 20, 3, 1, &rule);
...
int switchAddFdb(struct ether_addr *mac, swBitsMap map)
{
	...
	entry.addr = *mac;
	entry.port.map = map;
	return switchCommandExcute(SW_API_FDB_RESV_ADD, 0, &entry);
}
...
int switchClearFdb(struct ether_addr *mac, swBitsMap map)
{
	...
	entry.addr = *mac;
	entry.port.map = map;
	return switchCommandExcute(SW_API_FDB_RESV_DEL, 0, &entry);
}

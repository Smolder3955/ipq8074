...
void dbgInit(void)
{
    ...
    /* dbgEtherAddrSet may have been called first... */
    if (!dbgS.EtherAddr)
        dbgS.EtherAddr = strdup("");        /* default, don't use */
...
/*-F- dbgEtherAddrSet -- set ethernet addres to use in diagnostic messages.
 * This does need to be same address as actual br-lan address
 */
void dbgEtherAddrSet() {
#define ETHER_ADDR_LEN 18
    int socketFd;
    struct ifreq buf;

    if(dbgS.EtherAddr)free(dbgS.EtherAddr);

    socketFd = socket(PF_INET, SOCK_DGRAM, 0);
    if (socketFd == -1) {
        dbgDebug(DBGERR, "socket(...) failed");
        return;
    }
    memset(&buf, 0x00, sizeof(buf));
    strlcpy(buf.ifr_name, "br-lan", sizeof(buf.ifr_name));
    ioctl(socketFd, SIOCGIFHWADDR, &buf);
    close(socketFd);

    dbgS.EtherAddr = malloc(ETHER_ADDR_LEN+1);
    if(!dbgS.EtherAddr)
    {
        dbgDebug(DBGERR, "Malloc failed");
        return;
    }
    memset(dbgS.EtherAddr, 0, ETHER_ADDR_LEN);
    snprintf(dbgS.EtherAddr, ETHER_ADDR_LEN, "%.2X:%.2X:%.2X:%.2X:%.2X:%.2X",
...
/*-F- dbgParseCmdFromBuffer -- parse commands from buffer.
 * Wrapper to format Commands that are applied to diag or dbg or cmd modules.
 * dbg level command <modulename> and <level> can be separated by whitespace and/or
 * equal sign.
 * Diagnostics commands should be separated by whitespaces.
 * All other commands could be used as it is.
 *
 * NOTE NOTE the passed buffer is scribbled over (within the bounds
 * of the original string).
 */
void dbgParseCmdFromBuffer(char *B)
{
    diag_parseCmd(B);
}
        ...
        ...)    /* printf-like args */
{
    ...
    for (Out = &dbgS.Out; ;) {
        if (LevelName) {
            ...
            snprintf(str, LOG_BUFFER_SIZE, "%s %s%s %-8s %-5s: ",
                dbgS.EtherAddr, TimeBuf, dbgS.ProgramName, MP->ModuleName, LevelName);
        } else {
            fprintf(Out->OutF, "%s%s %-8s %-5d: ", 
                TimeBuf, dbgS.ProgramName, MP->ModuleName, Level);
            snprintf(str, LOG_BUFFER_SIZE, "%s %s%s %-8s %-5d: ",
                dbgS.EtherAddr, TimeBuf, dbgS.ProgramName, MP->ModuleName, Level);
        }
        va_start(ArgP, Format);
        vfprintf(Out->OutF, Format, ArgP);
        va_end(ArgP);
        va_start(ArgP, Format);
        vsnprintf(str + strlen(str), LOG_BUFFER_SIZE - strlen(str), Format, ArgP);
        va_end(ArgP);
        /* Add newline if one not given */
        if (Format[strlen(Format)-1] != '\n') {
            fprintf(Out->OutF, "\n");
            snprintf(str + strlen(str), LOG_BUFFER_SIZE - strlen(str), "\n");
        }
        diag_write(str, strlen(str));
        diag_finishEntry();

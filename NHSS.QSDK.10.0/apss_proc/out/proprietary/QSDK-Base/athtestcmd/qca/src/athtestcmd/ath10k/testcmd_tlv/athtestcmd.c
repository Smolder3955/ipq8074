...
void BoardDataCapture(unsigned char * respdata,unsigned int devid)
{
    ...
    if (BDCapture==0){
        ...
        if (devid == 0x50) {
            ...
            printf("Capturing Caldata for QC98xx by Qcmbr :: Size %d\n",dataSize);
            printf("FlashCaldata Address start : %d \n",(start_index*QC98XX_BLOCK_SIZE));
...
--rx <promis/filter/report> \n\
      ...
      --rxrate <rate index (default 11 [54Mbps])>\n\
...
--hwcal <fullresetcal/reset1/reset2/reset3/hwreset/initbl/fullcal/rxdco/rxfilt/rxiq/txiq/txcl/pkdet>\n\
      --loop <loop count>\n\
...
int main (int argc, char **argv)
{
    ...
    while (1) {
        ...
        case 'r':
            ...
            } else if (!strcmp(optarg, "promis2")) {
                value = TCMD_CONT_RX_PROMIS;
                printf(" Its cont Rx promis2 mode \n");
                miscFlags |= RX_STATUS_PER_RATE_MASK;
                miscFlags |= NF_IN_DBM_MASK;
            } else if (!strcmp(optarg, "filter2")) {
                value = TCMD_CONT_RX_FILTER;
                printf(" Its cont Rx  filter2  mode \n");
                miscFlags |= RX_STATUS_PER_RATE_MASK;
                miscFlags |= NF_IN_DBM_MASK;
            ...
            } else if(!strcmp(optarg, "report2")) {
               printf(" Its cont Rx report2 mode \n");
                tlvConstruct = TRUE;
                createCommand(_OP_GENERIC_NART_CMD);
                opCode = _OP_GENERIC_NART_CMD;
                value = M_RX_DATA_STOP_CMD_ID;
            ...
            rxSetMac = TRUE;  //Re-using this to indicate "rxMode" param added and to prevent re-adding in case of 'setbssid' and 'setmac' options
#endif //USE_TLV2
            opCode = _OP_RX;
            break;
        case 'k':
...
//#define M_EEEPROM_BLOCK_WRITE_ID 201
	...
	case TCMD_HWCAL:
        ...
        case TCMD_CAL_XTAL:       // lim_debug
            respNeeded = TRUE;
            useTLV1p0=FALSE;
            capOut = atoi(optarg) & 0x7f; //
            //if (value > 8) value = 0;
            if (argc > 3) capIn = (atoi(argv[3])) & 0x7f;
            else capIn = capOut + 32;
            if (argc > 4) ctrlFlag = atoi(argv[4]);
            else ctrlFlag = 0;
            //addParameterToCommand((A_UINT8 *)"param2",(A_UINT8 *)&value);
            if (ctrlFlag != 6)
                printf("sending TCMD_CAL_XTAL PARM_CAPOUT = %d, capin = %d, ctrlFlag =%d \n", capOut, capIn, ctrlFlag);  //lim_debug
            else
                printf("sending TCMD_CAL_XTAL timeout = %d, temperature = %d, ctrlFlag =%d \n", capOut, capIn, ctrlFlag);  //lim_debug

            // create a command stream, with example tx parms
            //pCmdStream = (TESTFLOW_CMD_STREAM_V2 *)createCmdRsp(CMD_XTALCALPROC, 6, PARM_CAPIN, 45, PARM_CAPOUT, 15, PARM_CTRLFLAG, value);
            pCmdStream = (TESTFLOW_CMD_STREAM_V2 *)createCmdRsp(CMD_XTALCALPROC, 6, PARM_CAPIN, capIn, PARM_CAPOUT, capOut, PARM_CTRLFLAG, ctrlFlag);
            cmdStreamLen = (sizeof(TESTFLOW_CMD_STREAM_HEADER_V2) + pCmdStream->cmdStreamHeader.length);
        ...
        case TCMD_RX_RATE:
            {
                A_UINT32 rowIndex=0, mask=0, rateBitMask=0;
                A_UINT32 rMask[2] = {0};
                char rateMask[10];
                value = atoi(optarg);
                if((value >= ATH_RATE_1M ) &&  (value <= ATH_RATE_VHT80_NSS4_MCS9 )) {
                    getMask2RateFromRateIndex(value, &rowIndex, &mask);    
                    rateBitMask = 1 << mask;
                    ...
                    //                printf("val:%d - %s ratemask:%d bitpos: %d", value, rateMask, rateBitMask, mask);
                    ...
                    printf("\nInvalid rate index argument. Kindly refer athtestcmd usage/help option for valid rate arguments. \
                             Default rate index 11 <54 Mbps> is set in FW \n");
                }
            }
        case '?':
            break;
    ...
    //UTF_CMD_SEND(buf,cmdStreamLen,respdata,((respNeeded) ? 1 : 0));
    UTF_CMD_SEND(buf,cmdStreamLen,respdata,&nrespdata,devid);
    printf("sent %d\n", cmdStreamLen);
    cmdReplyFunc(respdata);
...
void getMask2RateFromRateIndex (A_UINT32 index, A_UINT32 *rateMaskRow, A_UINT32 *rateMaskBit) {
	...
	*rateMaskRow = rateBitIndex/32;
	...
	//printf("\n getMask2RateFromRateIndex :  %d %d %d \n", rateBitIndex, *rateMaskRow, *rateMaskBit);

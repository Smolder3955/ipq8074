...
static char parmTypeTemp[PARM_TYPE_MAX][16]={"TYPE_SINGULAR", "TYPE_ARRAY"};
...
static char *makeValueStr(PARM_ONE *parm, char *arrayName)
{
    ...
    if ((TYPE_ARRAY == parm->parmType) && (parm->numValue == 0))
    {
...
static A_UINT32 genRandomNum(void)
{
    ...
    retRand = ((rand() % 0xffff) | ((rand() % 0xffff) << 16)) % 2999999999;
    return(retRand);
}
...
static A_BOOL readCmdRspDictH()
{
    ...
    {
        printf("Error in opening ../include/cmdRspParmsDict.h file.\n");
        return FALSE;
    }
    ...
    while(fgets(lineBuf, MAX_INPUT_LINE, fp) != NULL)
    {
        if (strstr(lineBuf, "typedef enum cmdCodeEnum {") == lineBuf)
        {
            ...
            while(fgets(lineBuf, MAX_INPUT_LINE, fp) != NULL)
            {
                ...
                if (strstr(pLine, "CMD_LAST,") == pLine)
                {
                    break; // done commands
                }
                else if (strstr(pLine, "CMD_") == pLine)
                {
                    strcpy (CmdCodes[numCmd++], pLine); 
                }
                else if (strstr(pLine, "#define") == pLine)
                {
        ...
        else if (strstr(lineBuf, "typedef enum parmCodeEnum {") == lineBuf)
        {
            ...
            while(fgets(lineBuf, MAX_INPUT_LINE, fp) != NULL)
            {
                ...
                if (strstr(pLine, "PARM_LAST,") == pLine)
                {
                    break; // done parmeters
                }
                else if (strstr(pLine, "PARM_") == pLine)
                {
...
static A_BOOL genData()
{
    ...
    for (idx=0;idx<NumCmdCodes;idx++) { 
        ...
        if (_strnicmp(pCmdCode, cmdBuf.code, MAX_CODE_LEN) == 0) {
            // cmd already present in the dictionary, what's going on
...
static A_BOOL genHandlerC(char *inFile)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        pCode = &pBuf->code[4]; //pass the prefix "CMD_"
        ...
        else
        {
            ...
            fprintf(fp, "    CMD_%s_PARMS  *%s = (CMD_%s_PARMS *)pParmsCommon;\n\n", pCode, ptrName, pCode);
            ...
            for (n = 0; n < pBuf->numParm; n++)
            {
                ...
                {
                    fprintf(fp, "    %s->%s = pParmDict[%s].v.%s;\n", ptrName, pParm[n].name, pParm[n].parm, GetValueTypeStr(&pParm[n]));
                }
                else //TYPE_ARRAY
                {
        ...
        {
            fprintf(fp, "    CMD_%s_PARMS *%s = (CMD_%s_PARMS *)pParms;\n\n", pCode, ptrName, pCode);
...
static A_BOOL genTlv2ApiH(char *cmdFile)
{
    ...
    char inFile[FILENAME_SIZE] = "../include/tlv2Api.h";
    char outFile[FILENAME_SIZE] = "output/tlv2Api.h";
    ...
    {
        if (strstr(lineBuf, "#endif // _TLV2_API_H_") == lineBuf)
        {
...
static A_BOOL genCmdHandlersH(char *cmdFile)
{
    ...
    while(fgets(lineBuf, MAX_INPUT_LINE, fIn) != NULL)
    {
        ...
        else if(strstr(lineBuf, "    } __ATTRIB_PACK cmdParmU;")) 
        {
            ...
            {
                sprintf(addLine, "        %s_PARMS\t\t cmd%sParms;\n", str2Upper(cmdBuf.code), &(cmdBuf.code[4]));
                fprintf(fOut, addLine);   
            }
            if (!rspCodeAlreadyIn && rspBuf.numParm)
            {
                sprintf(addLine, "        %s_PARMS\t\t rsp%sParms;\n", str2Upper(rspBuf.code), &(rspBuf.code[4]));
                fprintf(fOut, addLine);   
            }
        }
...
static A_BOOL genCmdRspDictC()
{
    ...
    if( (fpIn = fopen( "../common/cmdRspParmsDict.c", "r")) == NULL ) 
    {
        printf("Error in opening the ../common/cmdRspParmsDict.c file.\n");
        return FALSE;
    }
    if( (fpOut = fopen( "output/cmdRspParmsDict.c", "w")) == NULL ) 
    {
        printf("Error in opening the output/cmdRspParmsDict.c file.\n");
        if (fpIn) fclose(fpIn);
        return FALSE;
    }
...
static A_BOOL genCmdRspDictH()
{
    ...
    if( (fp = fopen( "output/cmdRspParmsDict.h", "w")) == NULL ) 
    {
        printf ("Error: could not generate cmdRspParmsDict.h\n");
        return FALSE;
    }

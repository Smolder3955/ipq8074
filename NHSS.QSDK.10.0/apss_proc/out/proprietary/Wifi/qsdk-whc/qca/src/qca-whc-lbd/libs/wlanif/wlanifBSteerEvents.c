...
struct wlanifBSteerEventsPriv_t {
    ...
    //netlink socket multicast support
    u_int8_t nlSteerMulticast;

   //flag to know ack based rssi support is enabled
...
wlanifBSteerEventsHandle_t wlanifBSteerEventsCreate(struct dbgModule *dbgModule,
    ...
    wlanifBSteerControlGetAckRssiEnable(state->bsteerControlHandle,
                                        &state->ackRssiEnable);

    wlanifBSteerControlGetModCounter(state->bsteerControlHandle,
                                        &state->logModCounter);
#if !SONLIB_SUPPORT_ENABLED
...
LBD_STATUS wlanifBSteerEventsHandleRemoteAssocUpdate(
    ...
    assocEvent.sta_class = wlanif_cap_unchanged;
...
LBD_STATUS wlanifBSteerEventsHandleRemoteClassGroupUpdate(
        wlanifBSteerEventsHandle_t state, const struct ether_addr *staAddr,
    ...
    wlanif_classGroupUpdateEvent_t classGroupEvent;
    lbCopyMACAddr(staAddr->ether_addr_octet,
                  classGroupEvent.sta_addr.ether_addr_octet);
    classGroupEvent.clientClassGroup = clientClassGroup;

    mdCreateEvent(mdModuleID_WlanIF, mdEventPriority_Low,
                  wlanif_event_classgroup_update, &classGroupEvent,
                  sizeof(classGroupEvent));
...
//When SonLib is enabled, NL socket is created in Son Lib init
    ...
    addr.nl_pid = getpid();
    wlanifBSteerControlGetNLSteerMulticast(
        state->bsteerControlHandle,
        &state->nlSteerMulticast);

    if (state->nlSteerMulticast)
        addr.nl_groups = 1;  // using multicast
    else
        addr.nl_groups = 0;  // using unicast
...
static void wlanifBSteerEventsHandleNodeAssociatedInd(
    ...
    dbgf(state->dbgModule, DBGINFO,
         ...
         event->data.bs_node_associated.isBeaconMeasurementSupported ? "RRM" : "",
         ...
         event->data.bs_node_associated.client_class_group);
    ...
    assocEvent.rrmStatus =
        event->data.bs_node_associated.isBeaconMeasurementSupported ? wlanif_cap_enabled :
                                                        wlanif_cap_disabled;
    assocEvent.sta_class =
        event->data.bs_node_associated.client_class_group;
...
static void wlanifBSteerEventsHandleSTAStatsInd(
    ...
    const lbd_bssInfo_t *bss) {
    size_t i;
#ifdef WIFISON_SUPPORT_ACK_RSSI
    static u_int32_t printcounter = 0;
#endif
    ...
    for (i = 0; i < stats->peer_count; i++) {
        ...
        if (state->staStatsCallback) {
            ...
            stats.per = peer->per;
#ifdef WIFISON_SUPPORT_ACK_RSSI
            // Use ack based rssi value for triggering steering.Replace rssi
            // value with ack based rssi value if ackRssiEnable is TRUE.
            if (state->ackRssiEnable) {
                stats.rssi = peer->ack_rssi;

                if (!(printcounter % state->logModCounter)) {
                    dbgf(state->dbgModule, DBGINFO,
                     "%s: " lbMACAddFmt(":") " peer->ack_rssi=%d, peer->rssi=%d, state->ackRssiEnable=%d\n",
                     __func__, lbMACAddData(peer->client_addr), peer->ack_rssi, peer->rssi, state->ackRssiEnable);
                    printcounter = 0;
                }
                printcounter++;
           }
...
static void wlanifBSteerEventsBufRdCB(void *cookie) {
    ...
    do {
        ...
        if (state->nlSteerMulticast) {
            /* In case of traffic separation, ignore messages sent for other process */
            if (hdr->nlmsg_pid != getpid())
                break;
        }

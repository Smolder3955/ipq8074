...
/**
 * @brief Iterate over all entries in the station database, invoking func
 *        for each one that matches the AP and channel IDs.
 *
 * A channel ID of LBD_CHANNEL_INVALID indicates to skip the check for
 * matching channel ID.
 *
 * @param [in] callback  the callback function
 * @param [in] apId  the serving AP Id to filter by
 * @param [in] channelId  the serving channel Id to filter by
 * @param [in] cookie  opaque parameter to provide in the callbacks
 *
 * @return LBD_OK if the iteration was successful; otherwise LBD_NOK
 */

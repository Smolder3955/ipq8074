...
# --------   ---       ------------------------------------------------------
# 11/22/18   gs        Support to add xbl sec when entry and segment offset are different
...
def main():
  ...
  parser.add_option("-i", "--binary input file",
                   action="store", type="string", dest="binary_input_file",
                   help="Input Binary file to merg")



  parser.add_option("-j","--binary file Lodable segment phyaddress",
                   action="store", type="string", dest="seg_physical_addr",
                   help="binary File Lodable segment physical address")


  parser.add_option("-e","--Mark enrty point in second merged ELF file",
                  action="store_true", dest="Phdr_Entry_identy",
                  help="Mark the Entry point in the second Merged ELF file")
  ...
  if  options.binary_input_file:
    if not options.seg_physical_addr:
      parser.error('Binary file Segment physical Address should be provided.')


  if options.Phdr_Entry_identy:
    phdr_Entry_flag = True
  else:
    phdr_Entry_flag = False
  ...
  if options.binary_input_file:
    binary_input_file =  options.binary_input_file
  else:
     binary_input_file = ""

  if options.seg_physical_addr:
    physical_addr = int(options.seg_physical_addr,16)
  else:
    physical_addr = 0
...
def merge_elfs(env, 
  ...
  if binary_input_file != "":
    bin_in_fp = mbn_tools.OPEN(binary_input_file,"rb")
...
#    print curr_phdr.p_filesz
  ...
  if elf_in_file_name2 != "":
    for i in range(elf_header2.e_phnum):
      ...
      if phdr_Entry_flag == True:
        if(curr_phdr.p_paddr == elf_header2.e_entry):
          new_phdr.p_type = new_phdr.p_type | 0x64000000
...
#     print curr_phdr.p_filesz
  ...
  if binary_input_file !="":
    ...
    if is_out_elf_64_bit == True:
      ...
      new_phdr.p_type   = 0x1
      new_phdr.p_offset = segment_offset
      new_phdr.p_vaddr  = physical_addr
      new_phdr.p_paddr  = physical_addr
      new_phdr.p_filesz = bin_file_len
      new_phdr.p_memsz  = bin_file_len
      new_phdr.p_flags  = 0x4
      new_phdr.p_align  = 0x1000
    else:
      # Converting from 64 to 32 elf requires data size validation
      # Note that there is an option to discard a segment if it is only ZI
    ...
    if physical_addr > 0xFFFFFFFF:
      if (zi_oob_enabled == True ) and (bin_file_len == 0):
       exit()
      else:
        print("ERROR: binary file physical Address too large for conversion.")
        exit()
    new_phdr.p_vaddr  = physical_addr
    new_phdr.p_paddr  = physical_addr

    if bin_file_len > 0xFFFFFFFF:
      print "ERROR: File 2 Filesz is too large for conversion."
      exit()
    new_phdr.p_filesz = bin_file_len
    new_phdr.p_memsz = bin_file_len
    ...
    # Copy the ENTIRE xbl_sec image
    bytes_written = mbn_tools.file_copy_offset(bin_in_fp,
                                               inp_data_offset,
                                               elf_out_fp,
                                               new_phdr.p_offset,
                                               new_phdr.p_filesz)

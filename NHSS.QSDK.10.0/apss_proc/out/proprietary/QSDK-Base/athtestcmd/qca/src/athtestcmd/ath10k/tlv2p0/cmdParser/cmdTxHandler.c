...
void* initTXOpParms(A_UINT8 *pParmsCommon, PARM_OFFSET_TBL *pParmsOffset, PARM_DICT *pParmDict)
{
    ...
    pTXParms->radioId = pParmDict[PARM_RADIOID].v.valU8;
    ...
    memset(pTXParms->pad3, 0, sizeof(pTXParms->pad3));
    for (i = 0; i < 3 ; i++)
    {
        pTXParms->pad3[i] = pParmDict[PARM_PAD3].v.ptU8[i];
    }
    memset(pTXParms->rateMask, 0, sizeof(pTXParms->rateMask));
    for (i = 0; i < 2 ; i++)
    {
    ...
    memset(pTXParms->pwrGainStart, 0, sizeof(pTXParms->pwrGainStart));
    for (i = 0; i < 16 ; i++)
    {
        pTXParms->pwrGainStart[i] = pParmDict[PARM_PWRGAINSTART].v.ptS32[i];
    }
    memset(pTXParms->pwrGainEnd, 0, sizeof(pTXParms->pwrGainEnd));
    for (i = 0; i < 16 ; i++)
    {
        pTXParms->pwrGainEnd[i] = pParmDict[PARM_PWRGAINEND].v.ptS32[i];
    }
    memset(pTXParms->pwrGainStep, 0, sizeof(pTXParms->pwrGainStep));
    for (i = 0; i < 16 ; i++)
    {
        pTXParms->pwrGainStep[i] = pParmDict[PARM_PWRGAINSTEP].v.ptS32[i];
    }
    memset(pTXParms->pwrGainStart11AC, 0, sizeof(pTXParms->pwrGainStart11AC));
    for (i = 0; i < 36 ; i++)
    {
        pTXParms->pwrGainStart11AC[i] = pParmDict[PARM_PWRGAINSTART11AC].v.ptS32[i];
    }
    memset(pTXParms->pwrGainEnd11AC, 0, sizeof(pTXParms->pwrGainEnd11AC));
    for (i = 0; i < 36 ; i++)
    {
        pTXParms->pwrGainEnd11AC[i] = pParmDict[PARM_PWRGAINEND11AC].v.ptS32[i];
    }
    memset(pTXParms->pwrGainStep11AC, 0, sizeof(pTXParms->pwrGainStep11AC));
    for (i = 0; i < 36 ; i++)
    {
        pTXParms->pwrGainStep11AC[i] = pParmDict[PARM_PWRGAINSTEP11AC].v.ptS32[i];
    }
    memset(pTXParms->bssid, 0, sizeof(pTXParms->bssid));
    for (i = 0; i < 6 ; i++)
    {
        pTXParms->bssid[i] = pParmDict[PARM_BSSID].v.ptU8[i];
    }
    memset(pTXParms->txStation, 0, sizeof(pTXParms->txStation));
    for (i = 0; i < 6 ; i++)
    {
        pTXParms->txStation[i] = pParmDict[PARM_TXSTATION].v.ptU8[i];
    }
    memset(pTXParms->rxStation, 0, sizeof(pTXParms->rxStation));
    for (i = 0; i < 6 ; i++)
    {
    ...
    fillParmOffsetTbl((A_UINT32)PARM_RADIOID, (A_UINT32)(((A_UINT32)&(pTXParms->radioId)) - (A_UINT32)pTXParms), pParmsOffset);
...
//#ifdef _DEBUG
    ...
    A_PRINTF("TXOp: dacGain %d\n", pTXParms->dacGain);    A_PRINTF("TXOp: dacGainEnd %d\n", pTXParms->dacGainEnd);    A_PRINTF("TXOp: dacGainStep %d\n", pTXParms->dacGainStep);    A_PRINTF("TXOp: paConfig %u\n", pTXParms->paConfig);

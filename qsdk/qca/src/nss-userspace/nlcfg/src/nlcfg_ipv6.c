...
static int nlcfg_ipv6_flow_add(struct nlcfg_param *param, struct nlcfg_param_in *match)
{
	...
	nss_nlipv6_init_rule(&nss_ctx, &nl_msg, NULL, nlcfg_ipv6_resp, NSS_IPV6_TX_CREATE_RULE_MSG);

	nss_nlipv6_init_conn_rule(&nl_msg.nim.msg.rule_create);

	/*
	 * extract selectors
	 */
	struct nlcfg_param *sub_params = &param->sub_params[NLCFG_IPV6_FLOW_ADD_SEL_SIP];
	error = nlcfg_ipv6_get_sel(sub_params, &nl_msg.nim.msg.rule_create.tuple);
	if (error) {
		return error;
	}
...
static int nlcfg_ipv6_flow_del(struct nlcfg_param *param, struct nlcfg_param_in *match)
{
	...
	nss_nlipv6_init_rule(&nss_ctx, &nl_msg, NULL, nlcfg_ipv6_resp, NSS_IPV6_TX_DESTROY_RULE_MSG);

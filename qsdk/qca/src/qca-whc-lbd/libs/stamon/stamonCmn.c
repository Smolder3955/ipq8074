...
LBD_STATUS stamon_init(void) {
    ...
    if (profileGetOptsIntArray(mdModuleID_StaMon,
        STAMON_HIGH_TX_RATE_XING_THRESHOLD,
        stamonCmnElementDefaultTable, value) == LBD_NOK) {
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        // Convert from the Kbps config value to Mbps
        stamonState.config.highTxRateCrossingThreshold[index] = value[index] / 1000;
    }    
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        stamonState.config.highRateRSSIXingThreshold[index] = value[index];
    }
    ...
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        stamonState.config.inactRSSIXingThreshold_DG[index] = value[index];
    }
    ...
    if (profileGetOptsIntArray(mdModuleID_StaMon,
        ...
        dbgf(stamonState.dbgModule, DBGERR,
             ...
             wlanif_band_24g, STAMON_LOW_AP_STEER_RSSI_XING_THRESHOLD_W2_KEY);
        return LBD_NOK;
    }
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        stamonState.config.apSteerLowRSSIXingThresholds[wlanif_band_24g][index] = value[index];
    }
    if (profileGetOptsIntArray(mdModuleID_StaMon,
        ...
        dbgf(stamonState.dbgModule, DBGERR,
             "[Band %u] Unable to parse %s",
             wlanif_band_5g, STAMON_LOW_AP_STEER_RSSI_XING_THRESHOLD_W5_KEY);
        return LBD_NOK;
    }
    for (index = 0; index < BSTEERING_MAX_CLIENT_CLASS_GROUP; index++) {
        stamonState.config.apSteerLowRSSIXingThresholds[wlanif_band_5g][index] = value[index];
    }
...
static void stamonCmnMakeSteerDecisionActive(stadbEntry_handle_t entry) {
    ...
    do {
        ...
        steeralg_rateSteerEligibility_e rateEligibility =
            steeralg_determineRateSteerEligibility(entry, staStats.lastTxRate, band);
...
LBD_BOOL stamonCmnIsEligibleForActiveUpgrade(stadbEntry_handle_t entry,
    ...
    if (rssi <= stamonState.config.highRateRSSIXingThreshold[clientClassGroup]) {
        dbgf(stamonState.dbgModule, DBGDEBUG,
             ...
             __func__, lbMACAddData(staAddr->ether_addr_octet), tx_rate, rssi,
             stamonState.config.highRateRSSIXingThreshold[clientClassGroup]);
        return LBD_FALSE;
    }

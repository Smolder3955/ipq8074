 ...
 *   order.
 *
 *   $Header: //components/rel/boot.bf/3.3.1/boot_images/core/storage/tools/deviceprogrammer_ddr/src/emmc/deviceprogrammer_storage_device.c#3 $
 *   $DateTime: 2017/04/11 22:02:31 $ $Author: pwbldsvc $
 *
 * YYYY-MM-DD   who     what, where, why
 * ----------   ---     ----------------------------------------------
 * 2016-10-05   wek     Add support for a partition erase
 * 2016-03-15   wek     Print the storage info inside parseable brackets.
...
boolean storageDeviceErase(storage_device_t *storedev, SIZE_T start_sector, SIZE_T num_sectors) {
    ...
    rc = sdcc_handle_erase(hsdev_handle, (uint32)start_sector, (uint32)num_sectors);
    if (rc != SDCC_NO_ERROR)
        return FALSE;
...
boolean storageDeviceFormat(storage_device_t *storedev) {
    ...
    return storageDeviceErase(storedev, 0, mem_info.card_size_in_sectors - 1);
}
...
boolean storageDeviceGetStorageInfo(storage_device_t *storedev) {
  ...
  logMessage("MFR_ID=%d", mem_info.mfr_id);
  logMessage("OEM_ID=%d", mem_info.oem_id);
  logMessage("Product name=%s", mem_info.prod_name);


  rc = sdcc_handle_get_fw_version (hsdev_handle, &fw_version);
  if (rc != SDCC_NO_ERROR){
    fw_version = 0;
    logMessage("SDCC Failed to get FW Version %d", rc);
  }
  logMessage("{&quot;storage_info&quot;: {&quot;total_blocks&quot;:%u, "
             "&quot;block_size&quot;:%u, &quot;page_size&quot;:%u, "
             "&quot;num_physical&quot;:%u, &quot;mfr_id&quot;:%u, "
             "&quot;serial_num&quot;:%u, "
             "&quot;fw_version&quot;:&quot;%llu&quot;,"
             "&quot;mem_type&quot;:&quot;eMMC&quot;, "
             "&quot;prod_name&quot;:&quot;%s&quot;"
             "}}",
             mem_info.card_size_in_sectors, mem_info.block_len, mem_info.block_len,
             mem_info.num_phy_partition_created, mem_info.mfr_id,
             mem_info.prod_serial_num, fw_version, mem_info.prod_name);

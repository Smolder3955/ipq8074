...
int nss_nlipv4_sock_cb(struct nl_msg *msg, void *arg)
{
	...
	case NSS_IPV4_TX_DESTROY_RULE_MSG:
	{
		...
		if (!resp.cb) {
			nss_nlsock_log_error("%d:no IPv4 response callback for cmd(%d)\n", pid, cmd);
			return NL_SKIP;
		}
...
int nss_nlipv4_sock_open(struct nss_nlipv4_ctx *ctx, void *user_ctx, nss_nlipv4_event_t event_cb)
{
	...
	memset(ctx, 0, sizeof(struct nss_nlipv4_ctx));
	 ...
	 */
	if (!event_cb) {
		nss_nlsock_log_error("%d:NSS IPv4 event listening is disabled\n", pid);
		return 0;
	}
	 ...
	 */
	error = nss_nlsock_listen(&ctx->sock, NSS_NLIPV4_MCAST_GRP);
	if (!error) {
		nss_nlsock_log_error("%d:unable to subscribe NSS IPv4 group, error(%d)\n", pid, error);
		goto fail;
	}
 ...
 *	send the IPv4 message asynchronously through the socket
 */
...
void nss_nlipv4_init_rule(struct nss_nlipv4_ctx *ctx, struct nss_nlipv4_rule *rule, void *data, nss_nlipv4_resp_t cb, enum nss_ipv4_message_types type)
{
	...
	struct nss_nlipv4_resp *resp = nss_nlcmn_get_cb_data(&rule->cm, family_id);
	assert(resp);

...
void* initRXOpParms(A_UINT8 *pParmsCommon, PARM_OFFSET_TBL *pParmsOffset, PARM_DICT *pParmDict)
{
    ...
    pRXParms->radioId = pParmDict[PARM_RADIOID].v.valU8;
    ...
    fillParmOffsetTbl((A_UINT32)PARM_RADIOID, (A_UINT32)(((A_UINT32)&(pRXParms->radioId)) - (A_UINT32)pRXParms), pParmsOffset);
...
//#ifdef _DEBUG
    ...
    A_PRINTF("RXOp: noiseFloor %d\n", pRXParms->noiseFloor);    for (i = 0; i < 2 ; i++)
    {
...
void* initRXRSPOpParms(A_UINT8 *pParmsCommon, PARM_OFFSET_TBL *pParmsOffset, PARM_DICT *pParmDict)
{
    ...
    // Populate the parm structure with initial values
    pRXRSPParms->radioId = pParmDict[PARM_RADIOID].v.valU8;
    pRXRSPParms->status = pParmDict[PARM_STATUS].v.valU8;
    pRXRSPParms->rxMode = pParmDict[PARM_RXMODE].v.valU8;
    pRXRSPParms->shortGuard = pParmDict[PARM_SHORTGUARD].v.valU8;
    pRXRSPParms->rssiInDBm1 = pParmDict[PARM_RSSIINDBM1].v.valS32;
    ...
    // Make up ParmOffsetTbl
    resetParmOffsetFields();
    fillParmOffsetTbl((A_UINT32)PARM_RADIOID, (A_UINT32)(((A_UINT32)&(pRXRSPParms->radioId)) - (A_UINT32)pRXRSPParms), pParmsOffset);
    ...
    fillParmOffsetTbl((A_UINT32)PARM_SHORTGUARD, (A_UINT32)(((A_UINT32)&(pRXRSPParms->shortGuard)) - (A_UINT32)pRXRSPParms), pParmsOffset);
    fillParmOffsetTbl((A_UINT32)PARM_RSSIINDBM1, (A_UINT32)(((A_UINT32)&(pRXRSPParms->rssiInDBm1)) - (A_UINT32)pRXRSPParms), pParmsOffset);
...
//#ifdef _DEBUG
    ...
    A_PRINTF("RXRSPOp: radioId %u\n", pRXRSPParms->radioId);
    A_PRINTF("RXRSPOp: status %u\n", pRXRSPParms->status);
    A_PRINTF("RXRSPOp: rxMode %u\n", pRXRSPParms->rxMode);
    A_PRINTF("RXRSPOp: shortGuard %u\n", pRXRSPParms->shortGuard);
    A_PRINTF("RXRSPOp: rssiInDBm1 0x%x\n", pRXRSPParms->rssiInDBm1);

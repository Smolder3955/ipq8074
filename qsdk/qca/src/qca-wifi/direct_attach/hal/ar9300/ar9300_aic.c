...
ar9300_aic_cal_post_process (struct ath_hal *ah)
{
    ...
    HDPRINTF(ah, HAL_DBG_BT_COEX, "(AIC) CAL_SRAM:\n");
    ...
    HDPRINTF(ah, HAL_DBG_BT_COEX, "(AIC) Post processing results:\n");
...
ar9300_aic_calibration(struct ath_hal *ah)
{
    ...
    while (cal_count)
    {
        ...
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(AIC) Start calibration #%d\n",
        ...
        for (i = 0; i < 10000; i++) {
            ...
            {
                HDPRINTF(ah, HAL_DBG_BT_COEX, "(AIC) Cal is done at #%d\n", i);
                break;
            }
            OS_DELAY(1);
        }
        ...
        if (i >= 10000) {
            HDPRINTF(ah, HAL_DBG_BT_COEX, "(AIC) Calibration failed.\n");
            break;
        }
    ...
    if (!cal_count) {
        HDPRINTF(ah, HAL_DBG_BT_COEX, "(AIC) Calibration failed2.\n");
    }
    ...
    ahp->ah_aic_enabled = ar9300_aic_cal_post_process(ah) ? true : false;

    HDPRINTF(ah, HAL_DBG_BT_COEX, "(AIC) ah_aic_enable = %d\n",
             ahp->ah_aic_enabled);
    return 0;
}
...
ar9300_aic_start_normal (struct ath_hal *ah)
{
    ...
    ahp->ah_aic_enabled = true;

    HDPRINTF(ah, HAL_DBG_BT_COEX, "(AIC) Start normal operation mode.\n");
    return 0;
}
#endif

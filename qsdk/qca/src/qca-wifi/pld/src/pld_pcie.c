...
static void pld_pcie_remove(struct pci_dev *pdev)
{
	...
	pld_context->ops->remove((struct device *)pdev, PLD_BUS_TYPE_PCIE);
...
/**
 * pld_pcie_fatal() - SSR fatal function for PCIE device
 * @pdev: PCIE device
 * @id: PCIE device ID
 *
 ...
 * Return: int
 */
static int pld_pcie_fatal(struct pci_dev *pdev,
			    const struct pci_device_id *id)
{
	struct pld_context *pld_context;
...
{
	return cnss_get_pci_dev_by_device_id(device_id);
}


int pld_pcie_rescan(void)
{
	return cnss_pcie_rescan();
}

void pld_pcie_remove_bus(void)
{
	cnss_pcie_remove_bus();
}
			 ...
			 enum pld_driver_mode mode, const char *host_version)
{
	...
	if (!config) {
		qdf_print("%s: Invalid wlan_cfg config", __func__);
		goto skip_cfg;
        }

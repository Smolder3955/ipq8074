...
/* maximum number of FTM measurements we request */
#define FTM_MAX_MEASUREMENTS	4

/* number of FTM measurements we process to get final RTT */
#define FTM_SET_SIZE		3
...
/* the RTT calculation algorithm in this sample (ftm_calculate_rtt)
 * only supports 3 measurements
 */
#if FTM_SET_SIZE != 3
#error unsupported FTM_SET_SIZE
#endif
...
{
	uint64_t t1[FTM_SET_SIZE], t2[FTM_SET_SIZE],
		 t3[FTM_SET_SIZE], t4[FTM_SET_SIZE];
	double ppmOffset[FTM_SET_SIZE], rttSum, tmpOffset, rtt;
	int i;

	if (state->num_ftm_meas < FTM_SET_SIZE) {
		log_printf("need %d measurements for RTT, got %d\n",
		       FTM_SET_SIZE, state->num_ftm_meas);
		return 0.0;
	}

	for (i = 0; i < FTM_SET_SIZE; i++) {
	...
	rtt = rttSum / (double)FTM_SET_SIZE;
				     ...
				     void *arg)
{
	...
	/* configure for single burst, 4 measurement exchanges
	   (3 effective results), burst duration == 250us */
	if (nla_put_u8(msg,
		       QCA_WLAN_VENDOR_ATTR_FTM_PARAM_MEAS_PER_BURST,
		       FTM_MAX_MEASUREMENTS) ||
	    nla_put_u8(msg,
...
static void ftm_run_ftmaoa_meas(struct ftmtest_state *state, void *arg)
{
	ftm_run_one_shot_session(state, arg);
	ftm_run_aoa_meas(state, arg);
}
...
void ftm_run_meas_loop(struct ftmtest_state *state,
		       ...
		       void *arg)
{
	struct timespec req, before = {0}, after = {0};
	uint64_t delta;
...
void ftm_cmd_run_ftmaoa(struct ftmtest_state *state, int argc, char *argv[])
{
	...
	ftm_run_meas_loop(state, args.count, args.delay, args.timeout,
			  ftm_run_ftmaoa_meas, args.mac_addr);
}
...
int main(int argc, char *argv[])
{
	...
	while ((f = getopt_long(argc, argv, "a:f:c:r:v", options, &optidx)) != EOF)
		switch(f) {
		case 'a':

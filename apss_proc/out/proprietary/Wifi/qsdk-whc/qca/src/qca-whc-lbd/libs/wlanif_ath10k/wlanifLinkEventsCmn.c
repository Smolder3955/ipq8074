...
void wlanifLinkEventsCmnProcessChannelChange(
        wlanifBSteerControlHandle_t state, const ath_netlink_bsteering_event_t  *event,
        wlanif_band_e band) {
    dbgf(state->dbgModule, DBGINFO,
            "%s: Channel change to %u", __func__, event->data.bs_channel_change.channelChanged);

    // If the band has actually changed on the VAP, we do not support
    // that so we will exit and let the process monitoring framework
    // restart us.
    wlanif_band_e newBand = wlanifMapFreqToBand(event->data.bs_channel_change.channelChanged);
    if (newBand != band) {
        dbgf(state->dbgModule, DBGERR,
                "%s: Change of band not supported in lbd; restarting",
                __func__);
    ...
    // Update the frequency in the BSteerControl record
    if (wlanifBSteerControlUpdateChannel(state,
                band,
                event->sys_index,
                event->data.bs_channel_change.channelChanged) != LBD_OK) {
        dbgf(state->dbgModule, DBGERR,
                "%s: Could not update channel in lbd; restarting",
                __func__);
        exit(1);
...
void wlanifLinkEventsCmnGenerateDisassocEvent(
        wlanifBSteerControlHandle_t state, const ath_netlink_bsteering_event_t *event) {
    wlanif_assocEvent_t assocEvent;
    lbCopyMACAddr(event->data.bs_node_disassociated.client_addr, assocEvent.sta_addr.ether_addr_octet);
    ...
    lbDbgAssertExit(state->dbgModule,
            wlanifBSteerControlGetBSSInfo(
                state,
                event->sys_index, &assocEvent.bss) != LBD_NOK);

    dbgf(state->dbgModule, DBGINFO, "%s: Client " lbMACAddFmt(":")
            " disassociated on "lbBSSInfoAddFmt(),
            __func__, lbMACAddData(event->data.bs_node_disassociated.client_addr),
            lbBSSInfoAddData((&assocEvent.bss)));
...
static void wlanifLinkEventsCmnHandleOperState(wlanifLinkEventsHandle_t state,
    ...
    if (dataLen == sizeof(operstate)) {
        ...
        if (changed) {
            ...
            wlanifLinkEventsCmnRestartUtilization(state->bsteerControlHandle, band);
        }
    } else {
        dbgf(state->dbgModule, DBGERR,

...
void* initRXSTATUSOpParms(A_UINT8 *pParmsCommon, PARM_OFFSET_TBL *pParmsOffset, PARM_DICT *pParmDict)
{
    ...
    pRXSTATUSParms->radioId = pParmDict[PARM_RADIOID].v.valU8;
    pRXSTATUSParms->stopRx = pParmDict[PARM_STOPRX].v.valU8;
    pRXSTATUSParms->freq = pParmDict[PARM_FREQ].v.valU16;

    // Make up ParmOffsetTbl
    resetParmOffsetFields();
    fillParmOffsetTbl((A_UINT32)PARM_RADIOID, (A_UINT32)(((A_UINT32)&(pRXSTATUSParms->radioId)) - (A_UINT32)pRXSTATUSParms), pParmsOffset);
...
//#ifdef _DEBUG
    ...
    A_PRINTF("RXSTATUSOp: radioId %u\n", pRXSTATUSParms->radioId);
...
void* initRXSTATUSRSPOpParms(A_UINT8 *pParmsCommon, PARM_OFFSET_TBL *pParmsOffset, PARM_DICT *pParmDict)
{
    ...
    // Populate the parm structure with initial values
    pRXSTATUSRSPParms->radioId = pParmDict[PARM_RADIOID].v.valU8;
    pRXSTATUSRSPParms->numOfReports = pParmDict[PARM_NUMOFREPORTS].v.valU8;
    memset(pRXSTATUSRSPParms->pad2, 0, sizeof(pRXSTATUSRSPParms->pad2));
    for (i = 0; i < 2 ; i++)
    {
    ...
    pRXSTATUSRSPParms->evm = pParmDict[PARM_EVM].v.valS32;
    ...
    // Make up ParmOffsetTbl
    resetParmOffsetFields();
    fillParmOffsetTbl((A_UINT32)PARM_RADIOID, (A_UINT32)(((A_UINT32)&(pRXSTATUSRSPParms->radioId)) - (A_UINT32)pRXSTATUSRSPParms), pParmsOffset);
...
//#ifdef _DEBUG
    ...
    A_PRINTF("RXSTATUSRSPOp: radioId %u\n", pRXSTATUSRSPParms->radioId);
    ...
    A_PRINTF("RXSTATUSRSPOp: evm %d\n", pRXSTATUSRSPParms->evm);    for (i = 0; i < 4 ; i++)
    {

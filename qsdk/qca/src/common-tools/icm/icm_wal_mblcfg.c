...
send:
    ...
    if ((channel <= 14) && (pconf->spectral_enab && picm->spectral_capable)) {
        ...
        snprintf(cmd, sizeof(cmd), "%s %s %s %1d", "iwconfig",
                 picm->dev_ifname, "channel", channel);
        ret = system(cmd);
...
int icm_wal_mblcfg_get_chan_rropinfo(ICM_INFO_T * picm)
{
    ...
    for (i = 0; i < rropinfo.num_rtplinst; i++) {
        /* picm->slist.chan_properties is indexed by primary channel number */
        if (rropinfo.rtpl[i].primary < ARRAY_LEN(picm->slist.chan_properties))
        {
            ICM_SET_CHANNEL_TX_POWER_TPUT(picm, rropinfo.rtpl[i].primary,
                    rropinfo.rtpl[i].txpower_throughput);
            ICM_SET_CHANNEL_TX_POWER_RANGE(picm, rropinfo.rtpl[i].primary,

...
typedef struct apacHyfi20SearchMid_t {
    apacBool_e valid[APAC_SEARCH_HIST_SZ];
    apacHyfi20WifiFreq_e freq;
    u16 mid[APAC_SEARCH_HIST_SZ];
    u8 nextHistoryIndex;            /* Index for the next write,
                                     * read is 0,1,..,(APAC_SEARCH_HIST_SZ-1) */
...
int apacHyfi20SendResponseR(apacHyfi20Data_t *ptrData, u8 *dest_mac, u8 freq, u16
        mid, apacBool_e isServiceMatched);
...
u8 apacMibGetVapIdxByRadioId(int raidoIdx, u8 *list, u8 *numbSTA);
...
int apacHyfi20CheckMid(u8 *alid, u16 mid, u8 isfrag);

...
int apacHyfi20SendAtfConfig(void *ptrData, int rep_index) {
    ...
    dprintf(MSG_MSGDUMP, "%s atfMsgLen : %d maxAtfMsgLen : %d atfBcstFlag : %d \n", __func__, atfMsgLen, maxAtfMsgLen, atfBcstFlag);
    if (!atfBcstFlag) {
...
int apacHyfi20ConfigureAtf(ATF_REP_CONFIG *apAtfConfig) {
    ...
    for(k = 0; k < apAtfConfig->radio_num_cfg; k++) {
        ...
        if( apAtfConfig->radioparams[k].sched_policy == APAC_ATF_SCHED_POLICY_STRICT) {
            ...
            ret = apacHyfi20ConfigAtf_radioparams(apAtfConfig->radioparams[k].interface, OL_ATH_PARAM_ATF_STRICT_SCHED, 1);
        } else if ( apAtfConfig->radioparams[k].sched_policy == APAC_ATF_SCHED_POLICY_FAIR) {
            dprintf(MSG_INFO,"Setting sched policy to Fair\n");
            ret = apacHyfi20ConfigAtf_radioparams(apAtfConfig->radioparams[k].interface, OL_ATH_PARAM_ATF_STRICT_SCHED, 0);
        }

        if( apAtfConfig->radioparams[k].obss_enable == APAC_ATF_OBSS_ENABLE) {
            dprintf(MSG_INFO,"Enabling OBSS\n");
            ret = apacHyfi20ConfigAtf_radioparams(apAtfConfig->radioparams[k].interface, OL_ATH_PARAM_ATF_OBSS_SCHED, 1);
        } else if ( apAtfConfig->radioparams[k].obss_enable == APAC_ATF_OBSS_DISABLE) {
            dprintf(MSG_INFO,"Disabling OBSS\n");
            ret = apacHyfi20ConfigAtf_radioparams(apAtfConfig->radioparams[k].interface, OL_ATH_PARAM_ATF_OBSS_SCHED, 0);
        }
        ...
        if( apAtfConfig->radioparams[k].group_sched_policy == APAC_ATF_GROUP_SCHED_STRICT) {
            ...
            ret = apacHyfi20ConfigAtf_radioparams(apAtfConfig->radioparams[k].interface, OL_ATH_PARAM_ATF_GROUP_POLICY, 1);
            ...
            ret = apacHyfi20ConfigAtf_radioparams(apAtfConfig->radioparams[k].interface, OL_ATH_PARAM_ATF_GROUP_POLICY, 0);
        }
    }
    return 0;
}

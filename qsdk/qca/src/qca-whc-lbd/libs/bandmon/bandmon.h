...
/**
 * @brief Get the number channels that are active on the network
 *
...
/**
 * @brief Get the set of channels that are active on the network
 *        and overloaded
 *
 ...
 *                      returned
 *
 * @return count of overloaded channels
 */
size_t bandmon_getOverloadedChannelList(lbd_channelId_t *channelList, size_t maxSize);

/**
 * @brief Check whether the given AP is in a blackout period to allow
 *        for updated measured utilization.
 *
 * @param apId [in]  the AP to check blackout period state
 *
...
/**
 * @brief Get the Id of the AP that is allowed to be offloaded.
 *
 * @return The Id of the AP that is allowed to be offloaded
 */
...
#define BANDMON_ALLOW_ZERO_LOCAL_CHANNELS_KEY "AllowZeroLocalChannels"
